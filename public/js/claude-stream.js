...
export async function streamClaude({ prompt, system = '', model = '', onToken, onDone, onError, maxTokens=1024, temperature=0.7 }) {
  try {
    await new Promise((resolve, reject) => {
      ...
      es.onmessage = (ev) => {
        try{
          ...
          if (data.done) { es.close(); onDone && onDone(); resolve(); }
          if (data.error) { es.close(); reject(new Error(data.error)); }
        }catch{ /* ignore */ }
      };
    ...
    // Fallback (and show reason)
    if (onToken) onToken(`(SSE-Fallback: ${sseErr.message})\n`);
  }
  try {
