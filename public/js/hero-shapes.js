/* public/js/hero-shapes.js — readable bubbles + focus + slower motion */
(function(){
  ...
  function collides(x,y,r){
    for(const s of pool){ const dx=x-s.cx, dy=y-s.cy; if(Math.hypot(dx,dy) < (r+s.r+24)) return true; }
    return false;
  }
  ...
  function spawn(h){
    ...
    const size=B*lerp(0.28,0.52,Math.random());
    const r=size/2, pad=60; let cx,cy,ok=false,tries=24;
    while(tries--){ cx=lerp(pad,W-pad,Math.random()); cy=lerp(pad+40,H-pad-120,Math.random()); if(!collides(cx,cy,r)){ok=true;break;} }
    ...
    if(Array.isArray(window.__TICKER_ITEMS)&&window.__TICKER_ITEMS.length){
      ...
      el.addEventListener('click',()=>{
        ...
        const content=el.dataset.prompt||''; if(window.openAnswerPopup) openAnswerPopup(content, false, el.dataset.heading);
      });
    }

    const AX=lerp(40,96,Math.random()), AY=lerp(36,82,Math.random());
    const speed=lerp(0.003,0.007,Math.random()); // slower motion
    const TX=lerp(520,840,Math.random()), TY=lerp(560,900,Math.random());
