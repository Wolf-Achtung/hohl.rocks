// public/js/hero-shapes.js — Jellyfish Motion Bubbles (readability+)
// Changes: removed inline blur (CSS handles blur on ::before so text stays crisp),
// minor polish and safe defaults.
...
function spawnOne() {
  ...
  // Blur is handled by CSS .shape::before; we keep the element itself sharp.
  ...
  el.addEventListener('click', () => {
    ...
    document.querySelectorAll('.shape').forEach(s => { if (s !== el) s.style.opacity = '0.33'; });
    setTimeout(() => document.querySelectorAll('.shape').forEach(s => { if (s !== el) s.style.opacity = '0.42'; }), 1200);
    ...
    if (action === 'input') {
      openInputBubble({ title: label, explain, placeholder, prompt, threadId });
    } else {
      openAnswerPopup({ title: label, explain, content: prompt });
    }
  });
