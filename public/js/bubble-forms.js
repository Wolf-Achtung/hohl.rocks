...
(() => {
  if (!document.getElementById("bubble-forms-css")) {
    const s = document.createElement("style"); s.id = "bubble-forms-css";
    s.textContent = `
      .bf-backdrop{position:fixed;inset:0;background:rgba(6,10,14,.35);backdrop-filter:blur(6px);z-index:1700}
      .bf-box{position:fixed;left:50%;top:4%;transform:translateX(-50%);width:min(1100px,94vw);background:rgba(12,16,22,.68);
        border:1px solid rgba(255,255,255,.18);border-radius:16px;color:#eaf2ff;z-index:1750}
      .bf-hd{display:flex;justify-content:space-between;align-items:flex-start;padding:14px 16px 8px}
      ...
      .bf-body{padding:12px 16px 16px}
  ...
  function closeAll() {
    document.querySelector(".bf-box")?.remove();
    document.querySelector(".bf-backdrop")?.remove();
  }

  // ESC zum Schließen aller Formular-Popups. Ein globaler Listener verhindert,
  // dass der Nutzer im Textfeld „stecken“ bleibt.
  document.addEventListener("keydown", (ev) => {
    if (ev.key === "Escape") closeAll();
  });

  function openInputBubble(title, basePrompt = "", options = {}) {
    const backdrop = document.createElement("div"); backdrop.className = "bf-backdrop"; backdrop.addEventListener("click", closeAll);
    ...
    left.appendChild(h3);
    if (options?.explain) { const p = document.createElement("p"); p.className = "explain"; p.textContent = options.explain; left.appendChild(p); }
    ...
    bClose.addEventListener("click", closeAll);
    ...
    const ta = document.createElement("textarea"); ta.className = "bf-textarea"; ta.placeholder = options?.placeholder || "Stichpunkte …";
    const send = document.createElement("button"); send.className = "bf-send"; send.textContent = "Senden";
    const row = document.createElement("div"); row.className = "bf-row"; row.appendChild(ta); row.appendChild(send);
    const out = document.createElement("div"); out.className = "bf-out";
    body.appendChild(row); body.appendChild(out);

    box.appendChild(hd); box.appendChild(body); document.body.appendChild(box);
    ...
    send.addEventListener("click", async () => {
      ...
      await window.Claude.stream(prompt, {
        threadId: options?.threadId, system: options?.system,
