...
export function openAnswerPopup({ title = 'Info', explain = '', content = '' } = {}) {
  closeAll();
  const overlay = el('div', 'popup'); const box = el('div', 'popup-inner');
  const h = el('div', 'popup-title', title); const x = el('button', 'popup-close', '×'); x.addEventListener('click', closeAll);
  const exp = explain ? el('div', 'popup-explain', explain) : null;
  const body = el('div', 'popup-body'); const pre = el('pre', 'answer'); pre.textContent = content;
  const actions = el('div', 'popup-actions'); const cpy = el('button', 'btn', 'Kopieren'); cpy.addEventListener('click', () => copyToClipboard(pre.textContent));
  const ok = el('button', 'btn btn-primary', 'Schließen'); ok.addEventListener('click', closeAll);
  actions.append(cpy, ok); body.append(pre); box.append(h, x); if (exp) box.append(exp); box.append(body, actions); overlay.append(box); root.append(overlay);
}
...
export function openInputBubble({ title = 'Frage', explain = '', placeholder = '', prompt = '', threadId = '' } = {}) {
  ...
  const out = el('pre', 'answer'); const actions = el('div', 'popup-actions');
  const send = el('button', 'btn btn-primary', 'Senden'); const cpy = el('button', 'btn', 'Kopieren'); cpy.disabled = true;
  const cancel = el('button', 'btn', 'Abbrechen'); actions.append(send, cpy, cancel);
  body.append(ta, out); box.append(h, x); if (exp) box.append(exp); box.append(body, actions); overlay.append(box); root.append(overlay);
