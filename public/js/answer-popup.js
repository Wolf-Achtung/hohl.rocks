// public/js/answer-popup.js — Unified Popups with Rich Content support
...
function basePopup({ title = 'Info', explain = '' } = {}) {
  ...
  box.append(h, x);
  if (explain) box.append(el('div', 'popup-explain', explain));
  ...
  return { overlay, box, body, cpy, ok };
}

export function openAnswerPopup({ title = 'Info', explain = '', content = '' } = {}) {
  closeAll();
  const ui = basePopup({ title, explain });
  const pre = el('pre', 'answer');
  pre.textContent = content;
  ui.body.append(pre);
  ui.cpy.addEventListener('click', () => copyToClipboard(pre.textContent));
}

export function openCustomPopup({ title = 'Info', explain = '', contentNode } = {}) {
  closeAll();
  const ui = basePopup({ title, explain });
  const container = el('div', 'answer rich');
  if (contentNode) container.append(contentNode);
  ui.body.append(container);
