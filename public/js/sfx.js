...
function clearVoices(){
  voices.forEach(v=>{ try{v.osc.stop();}catch{} try{v.osc.disconnect();}catch{} });
  voices=[];
}

const CHORDS = [
  // Warm & harmonisch: Am(add9), Fmaj7, Cmaj7, Gadd9
  [220, 261.63, 329.63, 440*1.1225], // A, C, E, add9(B)
  [174.61, 220, 261.63, 349.23],     // F, A, C, F
  [130.81*2, 164.81*2, 196*2, 246.94*2], // C, E, G, B (hoch)
  [196, 246.94, 293.66, 392]         // G, B, D, G
];
...
function playChord(idx){
  ...
  lfo.start();
  voices.push({osc:lfo,gain:lfoGain,filter:null});
}

function startAmbient(){
  const c=ensureCtx(); if(!c) return;
  stopAmbient();
  let idx=0; playChord(idx);
  intervalId=setInterval(()=>{ idx=(idx+1)%CHORDS.length; playChord(idx); }, 12000); // alle 12s Wechsel
}
...
if(btn){
  btn.addEventListener('click',()=>{ sfx.set(!sfx.enabled()); });
  btn.setAttribute('aria-pressed', String(enabled));
  if(enabled){ setTimeout(()=>sfx.set(true),0); }
}
