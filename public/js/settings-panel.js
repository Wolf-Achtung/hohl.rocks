...
function openPanel(){
  ...
  const inner = el('div', { class: 'popup-inner' });
  const head = el('div', { class: 'popup-header' }, [
    el('h3', { text: t('settings') }),
    (() => { const b = el('button', { class: 'btn btn-secondary', text: t('close') }); b.onclick = () => wrap.remove(); return b; })()
  ]);
