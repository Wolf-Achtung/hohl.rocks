...
function openPanel(){
  ...
  const modelRow=el('div',{},[ el('label',{for:'set-model',text:t('model')}), (()=>{const s=el('select',{id:'set-model'}); fetch('/api/models').then(r=>r.ok?r.json():Promise.reject(new Error('HTTP '+r.status))).then(list=>(list.models||list||DEFAULT_MODELS).forEach(m=>{const o=document.createElement('option'); o.value=m.id||m.name||m; o.textContent=m.display||m.id||m.name||m; s.appendChild(o);})).catch(()=>DEFAULT_MODELS.forEach(m=>{const o=document.createElement('option'); o.value=m.id; o.textContent=m.display; s.appendChild(o);})).finally(()=>{ if(settings.model) s.value=settings.model; }); return s; })() ]);
