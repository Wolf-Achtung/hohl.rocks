/* public/js/hero-shapes.js — bigger, readable, collision-aware bubbles + focus */
(function(){
  ...
  const pool=[]; // track live bubbles for simple collision-avoidance

  // Hit-test circles (approx)
  function collides(x,y,r){
    for(const s of pool){
      const dx = (x - s.cx), dy = (y - s.cy);
      const dist = Math.hypot(dx,dy);
      if (dist < (r + s.r + 24)) return true;
    }
    return false;
  }
  ...
  function spawn(h){
    ...
    const r = size/2;
    // find a position that doesn't collide (try 24 attempts)
    let cx, cy, ok=false, tries=24, pad=60;
    while(tries-- > 0){
      cx = lerp(pad, W-pad, Math.random());
      cy = lerp(pad+40, H-pad-120, Math.random());
    ...
    if(Array.isArray(window.__TICKER_ITEMS)&&window.__TICKER_ITEMS.length){
      ...
      el.addEventListener('click',()=>{
        ...
        const p=el.dataset.prompt||''; if(window.openAnswerPopup) window.openAnswerPopup(p, false, it.label);
      });
    }

    // Motion: slower & smoother (Lissajous-ish)
    ...
    setTimeout(()=>{
      el.style.opacity='0';
      setTimeout(()=>{ cancelAnimationFrame(el.__raf); el.remove();
        const i=pool.indexOf(state); if(i>=0) pool.splice(i,1);
        spawn(h);
      }, 2400);
    }, life);
  }
  ...
  function init(){
    const h=holder();
    const target = innerWidth<820 ? 7 : 10;
    for(let i=0;i<target;i++) setTimeout(()=>spawn(h), 400+i*900);
    // Remove the rotate-tip pre-msg after 8s if present
