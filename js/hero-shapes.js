/* hero-shapes.js — Neon Bubbles with label contrast fix & smooth motion */
(function(){
  ...
  function spawn(h){
    ...
    const size=B*lerp(0.22,0.46,Math.random());
    const AX=lerp(48,110,Math.random()), AY=lerp(42,95,Math.random());
    const speed=lerp(0.018,0.065,Math.random());
    const TX=lerp(220,360,Math.random()), TY=lerp(240,380,Math.random());
    ...
    // SVG with blend on SVG only (label unaffected)
    ...
    if(Array.isArray(window.__TICKER_ITEMS)&&window.__TICKER_ITEMS.length){
      ...
      Object.assign(lbl.style,{position:'absolute',left:'50%',top:'50%',transform:'translate(-50%,-50%)',color:'#0a1118',textAlign:'center',
        pointerEvents:'none',fontSize:'13px',fontWeight:'800',letterSpacing:'0.2px',
        background:'rgba(255,255,255,.35)',padding:'4px 8px',borderRadius:'10px',
        textShadow:'0 1px 2px rgba(255,255,255,.6), 0 0 10px rgba(255,255,255,.35)'});
    ...
    function draw(){
      ...
      el.style.opacity=(0.92*env).toFixed(2);
