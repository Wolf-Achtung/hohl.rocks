...
// In‑Memory Threads (4h TTL, max 500)
  ...
  try {
    ...
    if (image) {
      messages = [
        ...
        ...hist,
        { role: 'user', content: [
          { type: 'input_text', text: String(prompt || 'Bitte analysiere dieses Bild.') },
  ...
  try {
    ...
    for (const m of models) {
      const upstream = await fetch(API_URL, {
        ...
        body: JSON.stringify({ model: m, stream: true, max_tokens: 1200, temperature: 0.7, system,
          messages: toMessages(touchThread(id), prompt, system) })
      });
        ...
        while ((ix = buf.indexOf('\n\n')) >= 0) {
          ...
          try {
            ...
            if (ev?.type === 'content_block_delta' && ev?.delta?.text) {
              sent = true; acc += ev.delta.text;
              res.write(`event: delta\ndata: ${JSON.stringify({ text: ev.delta.text })}\n\n`);
            }
            if (ev?.type === 'message_stop') {
              pushMsg(id, 'user', prompt); pushMsg(id, 'assistant', acc);
              onClose(); res.write(`event: done\ndata: {}\n\n`); return res.end();
            }
          } catch { /* ignore */ }
        }
      }
