...
const TTL = 4 * 60 * 60 * 1000; const MAX = 500;
const SESS = new Map();
function touchThread(id) {
  const now = Date.now(); let t = SESS.get(id);
  if (!t) { t = { t: now, m: [] }; SESS.set(id, t); }
  t.t = now; const tooOld = now - TTL;
  if (SESS.size > MAX) { const arr = [...SESS.entries()].sort((a, b) => a[1].t - b[1].t); for (let i = 0; i < arr.length - MAX; i++) SESS.delete(arr[i][0]); }
...
const toMessages = (hist, user, system) => {
  ...
  msgs.push(...hist.slice(-40)); msgs.push({ role: 'user', content: String(user) }); return msgs;
};
...
router.post('/claude', express.json(), async (req, res) => {
  try {
    ...
    const hist = touchThread(id); let messages;
    if (image) {
      messages = [
        ...(system ? [{ role: 'system', content: system }] : []), ...hist,
        { role: 'user', content: [{ type: 'input_text', text: String(prompt || 'Bitte analysiere dieses Bild.') }, { type: 'input_image', image_url: image }] }
      ];
    } else messages = toMessages(hist, prompt, system);
    ...
    if (!r.ok) { const txt = await r.text().catch(() => ''); return res.status(502).json({ error: 'Claude upstream', status: r.status, body: txt }); }

    const j = await r.json(); const text = (j?.content || []).map(c => c?.text || '').join('');
    pushMsg(id, 'user', prompt); pushMsg(id, 'assistant', text);
...
router.post('/claude-json', express.json(), async (req, res) => {
  try {
    ...
    const r = await fetch(API_URL, { method: 'POST', headers: headers(),
      body: JSON.stringify({
        model, max_tokens: 900, temperature: 0.4, stream: false,
        system: 'Antworte NUR mit gültigem JSON ohne erklärenden Text.',
        messages: [{ role: 'user', content: String(prompt) }]
      })
    });
    if (!r.ok) { const txt = await r.text().catch(() => ''); return res.status(502).json({ error: 'Claude upstream', status: r.status, body: txt }); }
    const j = await r.json(); const raw = (j?.content || []).map(c => c?.text || '').join(''); let obj = null; try { obj = JSON.parse(raw); } catch {}
...
router.post('/claude-sse', express.json(), async (req, res) => {
  try {
    ...
    for (const m of models) {
      const upstream = await fetch(API_URL, { method: 'POST', headers: { ...headers({ accept: 'text/event-stream' }) },
      ...
      if (!upstream.ok) { const txt = await upstream.text().catch(() => ''); if (upstream.status === 404) continue;
      ...
      while (true) {
        ...
        while ((ix = buf.indexOf('\n\n')) >= 0) {
          ...
          try {
            ...
            if (ev?.type === 'content_block_delta' && ev?.delta?.text) { sent = true; acc += ev.delta.text; res.write(`event: delta\ndata: ${JSON.stringify({ text: ev.delta.text })}\n\n`); }
            if (ev?.type === 'message_stop') { pushMsg(id, 'user', prompt); pushMsg(id, 'assistant', acc); onClose(); res.write('event: done\ndata: {}\n\n'); return res.end(); }
          } catch {}
        }
      }
      onClose(); break;
