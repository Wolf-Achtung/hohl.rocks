...
function ensureKey(){
  if (!API_KEY) { const e = new Error('ANTHROPIC_API_KEY fehlt'); e.status = 503; throw e; }
}
...
router.post('/api/claude', async (req, res) => {
  try {
    const { prompt = '', system = '', model = DEFAULT_MODEL, maxTokens = 1024, temperature = 0.7 } = req.body || {};
...
router.get('/chat-sse', async (req, res) => {
  try{
    ...
    const max_tokens = Number(req.query.maxTokens || 1024);
    const temperature = Number(req.query.temperature || 0.7);
    ...
    const r = await fetch('https://api.anthropic.com/v1/messages', {
      method: 'POST', headers: { 'content-type': 'application/json', 'x-api-key': API_KEY, 'anthropic-version': '2023-06-01' }, body: JSON.stringify(body)
    });
    ...
    res.end();
  } catch (e) { res.write(`data: ${JSON.stringify({ error: e.message || String(e) })}\\n\\n`); res.end(); }
});
